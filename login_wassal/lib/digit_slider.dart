...
class DigitFlutterSlider extends StatefulWidget {
  final Key key;
  final Axis axis;
  final double handlerWidth;
  final double handlerHeight;
  final DigitFlutterSliderHandler handler;
  final DigitFlutterSliderHandler rightHandler;
  ...
  final List<DigitFlutterSliderFixedValue> fixedValues;
  final bool rangeSlider;
  final bool rtl;
  final bool jump;
  final bool selectByTap;
  final List<DigitFlutterSliderIgnoreSteps> ignoreSteps;
  final bool disabled;
  final double touchSize;
  final bool visibleTouchArea;
  final double minimumDistance;
  final double maximumDistance;
  final DigitFlutterSliderHandlerAnimation handlerAnimation;
  final DigitFlutterSliderTooltip tooltip;
  final DigitFlutterSliderTrackBar trackBar;
  final DigitFlutterSliderStep step;
  final DigitFlutterSliderHatchMark hatchMark;
  ...
  DigitFlutterSlider(
      ...
      this.tooltip,
      this.trackBar = const DigitFlutterSliderTrackBar(),
      this.handlerAnimation = const DigitFlutterSliderHandlerAnimation(),
      this.selectByTap = true,
      this.step = const DigitFlutterSliderStep(),
      this.hatchMark,
...
//        assert( fixedValues == null || (fixedValues != null && values.length > 1 && values[1] >= values[0] && values[1] <= 100), "When using fixedValues, you should set values within the range of fixedValues" ),
  ...
  @override
  _DigitFlutterSliderState createState() => _DigitFlutterSliderState();
}

class _DigitFlutterSliderState extends State<DigitFlutterSlider>
  ...
  DigitFlutterSliderTooltip _tooltipData;
  ...
  double _widgetMin;
  double _widgetMax;
  List<DigitFlutterSliderIgnoreSteps> _ignoreSteps = [];
  List<DigitFlutterSliderFixedValue> _fixedValues = [];
  ...
  @override
  void didUpdateWidget(DigitFlutterSlider oldWidget) {
    __isInitCall = false;
...
//                  ..._points,
  ...
  void initMethod() {
    ...
    switch (_tooltipData.direction) {
      case DigitFlutterSliderTooltipDirection.top:
        animationFinish = Offset(0, -1);
        break;
      case DigitFlutterSliderTooltipDirection.left:
        animationFinish = Offset(-1, 0);
        break;
      case DigitFlutterSliderTooltipDirection.right:
        animationFinish = Offset(1, 0);
        break;
    }
  ...
  void _drawHatchMark() {
    ...
    DigitFlutterSliderHatchMark hatchMark = DigitFlutterSliderHatchMark();
...
//      left = hatchMark.linesDistanceFromTrackBar - 3.62;
      ...
      if (widget.axis == Axis.horizontal) {
        if (hatchMark.linesAlignment ==
            DigitFlutterSliderHatchMarkAlignment.left) {
      ...
      for (int p = 0; p <= percent; p++) {
        DigitFlutterSliderSizedBox barLineBox = hatchMark.smallLine;
...
//        left = (p * distance) + _handlersPadding - labelBoxHalfSize - 0.5;
    ...
    if (hatchMark.labels != null && hatchMark.labels.length > 0) {
      ...
      double tr;
      for (DigitFlutterSliderHatchMarkLabel markLabel in hatchMark.labels) {
...
//          left = (tr * distance) + _handlersPadding - labelBoxHalfSize - 0.5;
  ...
  void _setParameters() {
    ...
    if (widget.fixedValues != null && widget.fixedValues.length > 0) {
      ...
      for (DigitFlutterSliderFixedValue fixedValue in widget.fixedValues) {
      ...
      for (double fixedPercent = 0; fixedPercent <= 100; fixedPercent++) {
        ...
        for (DigitFlutterSliderFixedValue fixedValue in widget.fixedValues) {
          if (fixedValue.percent == fixedPercent.toInt()) {
            ...
            if (fixedPercent > lowerIgnoreBound + 1 || lowerIgnoreBound == 0) {
              ...
              _ignoreSteps.add(DigitFlutterSliderIgnoreSteps(
        ...
        _fixedValues.add(DigitFlutterSliderFixedValue(
      ...
      if (!fixedV.contains(100)) {
        _ignoreSteps.add(
            DigitFlutterSliderIgnoreSteps(from: biggestPoint + 1, to: 101));
      }
    } else {
    ...
    DigitFlutterSliderTooltip widgetTooltip =
        widget.tooltip ?? DigitFlutterSliderTooltip();

    _tooltipData = DigitFlutterSliderTooltip();
    _tooltipData.boxStyle = widgetTooltip.boxStyle ??
        DigitFlutterSliderTooltipBox(
    ...
    _tooltipData.direction =
        widgetTooltip.direction ?? DigitFlutterSliderTooltipDirection.top;
  ...
  void _generateHandler() {
    /*Right Handler Data*/
    DigitFlutterSliderHandler inputRightHandler =
        widget.rightHandler ?? DigitFlutterSliderHandler();
  ...
  bool _leftHandlerIgnoreSteps(double tS) {
    ...
    if (_ignoreSteps.length > 0) {
      ...
      for (DigitFlutterSliderIgnoreSteps steps in _ignoreSteps) {
        if (((!widget.rtl) &&
  ...
  void _checkRangeStep(double realValue) {
    ...
    if (widget.step.rangeList != null) {
      for (DigitFlutterSliderRangeStep rangeStep in widget.step.rangeList) {
...
//    items      ..addAll(_points);
  ...
  Positioned _tooltip(
    ...
    if (numberFormat == '1.0') {
      signs = '₤';
    } else if (numberFormat == '2.0') {
      signs = '₤₤';
    } else if (numberFormat == '3.0') {
      signs = '₤₤₤';
    } else if (numberFormat == '4.0') {
      signs = '₤₤₤₤';
    } else if (numberFormat == '5.0') {
      signs = '₤₤₤₤₤';
    }
    List<Widget> children = [
      prefix,
      Text(signs, style: _tooltipData.textStyle),
      suffix,
    ];
    ...
    if (_tooltipData.direction == DigitFlutterSliderTooltipDirection.top) {
...
class _MakeHandler extends StatelessWidget {
  ...
  final double height;
  final GlobalKey id;
  final DigitFlutterSliderHandler handlerData;
  final bool visibleTouchArea;
  ...
  Widget build(BuildContext context) {
    ...
    DigitFlutterSliderHandler handler =
        handlerData ?? DigitFlutterSliderHandler();
...
class DigitFlutterSliderHandler {
...
class DigitFlutterSliderTooltip {
  ...
  DigitFlutterSliderTooltipDirection direction;
  DigitFlutterSliderTooltipPositionOffset positionOffset;

  DigitFlutterSliderTooltip({
    this.custom,
    this.format,
    this.textStyle,
    this.boxStyle,
...
class DigitFlutterSliderTooltipPositionOffset {
  double top;
  double left;
  double right;
  double bottom;

  DigitFlutterSliderTooltipPositionOffset(
...
class DigitFlutterSliderTooltipBox {
  final BoxDecoration decoration;
  final BoxDecoration foregroundDecoration;
  final Matrix4 transform;

  const DigitFlutterSliderTooltipBox(
...
class DigitFlutterSliderTrackBar {
  ...
  const DigitFlutterSliderTrackBar({
...
class DigitFlutterSliderIgnoreSteps {
  final double from;
  final double to;

  DigitFlutterSliderIgnoreSteps({this.from, this.to})
...
class DigitFlutterSliderFixedValue {
  final int percent;
  final dynamic value;

  DigitFlutterSliderFixedValue({this.percent, this.value})
      : assert(
...
class DigitFlutterSliderHandlerAnimation {
  final Curve curve;
  final Curve reverseCurve;
  final Duration duration;
  final double scale;

  const DigitFlutterSliderHandlerAnimation(
...
class DigitFlutterSliderHatchMark {
  bool disabled;
  double density;
  double linesDistanceFromTrackBar;
  double labelsDistanceFromTrackBar;
  List<DigitFlutterSliderHatchMarkLabel> labels;
  DigitFlutterSliderSizedBox smallLine;
  DigitFlutterSliderSizedBox bigLine;
  DigitFlutterSliderSizedBox labelBox;
  DigitFlutterSliderHatchMarkAlignment linesAlignment;
  bool displayLines;

  DigitFlutterSliderHatchMark(
      {this.disabled = false,
      this.density = 1,
      ...
      this.smallLine,
      this.bigLine,
      this.linesAlignment = DigitFlutterSliderHatchMarkAlignment.right,
      this.labelBox,
      this.displayLines})
      : assert(disabled != null),
...
class DigitFlutterSliderHatchMarkLabel {
  final double percent;
  final Widget label;

  DigitFlutterSliderHatchMarkLabel({
    this.percent,
    this.label,
...
class DigitFlutterSliderSizedBox {
  final BoxDecoration decoration;
  final BoxDecoration foregroundDecoration;
  final Matrix4 transform;
  final double width;
  ...
  const DigitFlutterSliderSizedBox(
      {this.decoration,
      this.foregroundDecoration,
      this.transform,
...
class DigitFlutterSliderStep {
  final double step;
  final bool isPercentRange;
  final List<DigitFlutterSliderRangeStep> rangeList;

  const DigitFlutterSliderStep({
    this.step = 1,
...
class DigitFlutterSliderRangeStep {
  final double from;
  final double to;
  final double step;

  DigitFlutterSliderRangeStep({
    this.from,
    this.to,
    this.step,
...
enum DigitFlutterSliderTooltipDirection { top, left, right }
enum DigitFlutterSliderHatchMarkAlignment { left, right }
